...
Creado para desarrolladores frontend que están cansados de que `json-server` sea *stateless* (sin estado) y de que `MirageJS` se ejecute *dentro del navegador*.
...
* ⚡️ **Estado Persistente en Memoria:** Los cambios de `POST`, `PUT`, `PATCH` y `DELETE` se guardan *realmente* mientras el servidor esté corriendo.
...
`memstate-api` detectará tu `db.json` y creará automáticamente 5 rutas CRUD *stateful* para cada recurso (`/posts` y `/users`):
...
Añade un retraso global (en milisegundos) a *todas* las respuestas de la API.
